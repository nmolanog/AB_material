...
```{r qgraph1, echo=FALSE, results = 'asis',fig.width=8, fig.height=4, fig.cap="Distribucion de peso para mujeres"}
...
```{r fdpoisr, echo=FALSE, results = 'asis',fig.width=7, fig.height=4, fig.cap="Función de densidad para la variable número de hijos"}
dtf_pois<-data.frame(x=factor(0:11),y=dpois(0:11,3))
dtf_pois%>%ggplot(aes(x=x,y=y))+geom_point()+
  ylab("f(x)")+ theme_bw()+
  theme(axis.title.y = element_text(angle=0))+
...
Nótese que las funciones de densidad multimodales pueden ser simétricas (figura \@ref(fig:fdmultimod) A ) o asimétricas (figuras \@ref(fig:fdmultimod) B y C)