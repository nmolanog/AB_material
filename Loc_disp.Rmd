...
p11<-ggplot(data = data.frame(weight = weight_lim), aes(weight)) +
  ...
  annotate("text", x = 100+7, y = .03, 
           label = round(pnorm(xrange[2],mm,sdm)-pnorm(xrange[1],mm,sdm),3),parse=T , size=5 , fontface="bold")+
  annotate("text", x = 40-7, y = .0175, 
           label = round(pnorm(xrange[2],mm,sdm+15)-pnorm(xrange[1],mm,sdm+15),3),parse=T , size=5 , fontface="bold")+
...
p11
```
En la figura \@ref(fig:fdnormcompV) las dos funciones de densidad tienen el mismo valor del parametro $\mu$, sin embargo, la funcion de densidad roja tiene el parametro $\sigma^2=729$ mientras la negra tiene $\sigma^2=144$.