...
p11<-ggplot(data = data.frame(weight = weight_lim), aes(weight)) +
  ...
  geom_segment(aes(x=xrange,xend=xrange,y=0,yend=.035),linetype=2)+
  stat_function(fun = dnorm, args = list(mean = mm,sd=sdm),
                xlim =xrange,
                geom = "area",fill="grey",alpha=0.5)+
  ...
  scale_color_identity(name = "",
                          breaks = c("black", "red"),
                          labels = c(expression(paste(mu,"= 70, ",sigma^2,"= 144")),
                                     expression(paste(mu,"= 70, ",sigma^2,"= 729"))),
                          guide = "legend")+
  theme_bw()
p11
```
En la figura \@ref(fig:fdnormcompV) las dos funciones de densidad tienen el mismo valor del parametro $\mu$, sin embargo, la funcion de densidad roja tiene el parametro $\sigma^2= `r (sdm+15)^2`$ mientras la negra tiene $\sigma^2=`r (sdm)^2`$. Para la funcion de densidad roja se tiene que $P(50< W <90)= `r round(prb2,3)`$ mientras que para la negra, el mismo intervalo tiene probabilidad $P(50<W<90)= `r round(prb1,3)`$. En cual de las dos funciones de densidad hay mayor incertidumbre de observar valores entre $50$ y $90$? Claramente en la roja en donde $\sigma^2$ es mayor.

Consideremos ahora otro enfoque