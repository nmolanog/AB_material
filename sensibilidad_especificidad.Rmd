...
res<-kable(z0[sample(1:nrow(z0),10,replace =T),1:3],caption="Tabla de datos para el experimento de prueba diagnóstica",row.names =F)
...
res22<-kable(temp_table2,caption="Tabla de contingencia para pruebas diagnósticas")
...
resTUMB<-kable(z0[sample(1:nrow(z0),10,replace =T),c(-3)],caption="Tabla de datos con resultados del test para diferentes umbrales",row.names =F)
...
depCTUMB<-kable(desp_df,caption="Métricas de desempeño para diferentes umbrales",row.names =T)
...
AUCkab<-kable(auc_res[order(auc_res$AUC),],caption="AUC para las 4 moléculas",row.names =F)
