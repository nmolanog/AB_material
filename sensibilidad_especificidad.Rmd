...
Claramente deseamos estar lo mas cerca posible del punto B en donde la prueba diagnositca coinside 100% con el estandar de oro. Este es de hecho un de los criterios para buscar el mejor umbral: econtrar el valor de umbral para el cual se esta mas cerca de la esquina superior izquierda del plano cartesiano del espacio ROC. Estos calculos son computacionalemnte intensivos y por lo general son realizados por programas hechos a medida para tal fin. En la figura \@ref(fig:rocg2) presentamos la curva ROC para la prueba diagnostica de cancer, el valor de corte optimo para la consentracion de la molecula estudiada y su respecitva sensibilidad y espesificidad.
...
# Comparacion de multiples pruebas diagnosticas

La curva ROC tiene otros usos, ademas del visto anteriormente: encontrar el mejor umbral para construir una prueba diagnostica. La curva ROC puede ser usada tambien para comparar y evaluar el desempeño de multiples pruebas diagnosticas. Suponga que ahora tenemos 3 moleculas (m1,m2 y m3) que compiten para ser la prueba diagnostica para cancer. En la figura \@ref(fig:rocg3)A presentamos sus curvas ROC respectivas.
<center>
...
Debemos recordar que entre mas cerca se esta de la esquina superior izquierda, mejor es el desempeño de la prueba diagnostica (al menos en terminos de sensibilidad y especificidad). Teniendo en cuenta esto, las curvas de la figura \@ref(fig:rocg3)A se ordenan, en orden de desempeño, de la siguiente forma: m3,m1 y m2, siendo m3 la molecula con mejor desempeño y m2 la de peor desempeño. Sin embargo, las comparaciones no siempre son tan faciles. Estudiemos el caso de la figura \@ref(fig:rocg3)B.

Estas dos curvas tienen regiones en donde una es superior a la otra: para valores de $1-Esp$, entre $0$ y $2$ (es decir entre valores de $Esp$ de $0.8$ y $1$) la curva de la molecula 4 esta por ensima de la curva de la molecula 2, mientras que en valores de $Sens$ de $0.75$ a $1$ la curva de la molecula 2 esta por ensima de la curva de la molecula 4 (ver \@ref(fig:rocg3)B). Si se tuviera que escojer exclusivamente entre las moleculas 2 y 4 cual escogeria Usted? La respuesta es un *"depende"*. Claramente la molecula 2 tiene un mejor desempeño en sensibilidad, mientras que la molecula 4 tiene un mejor desempeño en terminos de especificidad. 

Suponga que el tratamiento para la enfermedad en cuestion, no tiene efectos secundarios importantes para personas sanas, en ese caso seria adecuado usar la molecula 2 como prueba diagnostica ya que garantiza una alta sensibilidad, sin embargo su menor especificidad (es decir que se diagnosticaran mas sanos como enfermos) no tendra implicaciones importantes en la salud de los pacientes realmente sanos. Esto es pensando que aquellos pacientes realmente sanos que se diagnostican, segun la prueba diagnostica, como enfermos recibiran el tratamiento cuando realmente no lo nesecitan, y como este tratamiento es inocuo para ellos, bien vale la pena correr ese riesgo.

Sin embargo, si el tratamiento es costoso, invasivo y presenta importantes efectos secundarios, existira la preocupacion de exponer a sujetos sanos a dicho tratamiento, por lo cual se preferira no equivocarse con ellos y por ende, se optaria por la molecula 4, que garantiza una mayor especificidad, claro esta, a costa de un menor sensibilidad.

Como se puede apreciar, la seleccion de la molecula 2 o 4 dependera de los riesgos y costos asociados al tratamiento de la enfermedad, consideraciones que llevan al objetivo de maximizar la senseibilidad o la especificidad en uno y otro caso.

# Area bajo la curva ROC

Como vimos en la seccion anterior, entre mas cerca este la curva ROC de la esquina superior izquierda, mejor es el desempeño de la prueba diagnostica. Sin embargo no siempre es facil dicernir que curva esta mas cerca que otra, como lo observamos en el caso de la figura \@ref(fig:rocg3)B. Es nesezario entonces tener un valor objetivo de que tan cerca se esta de la esquina superior derecha, y para esto se puede usar el area bajo la curva ROC (AUC por sus siglas en ingles). Un clasificador perfecto tendria una curva triangular con un area bajo la curva de 1 (el area total de cuadrado delimitado por 1 de $Sens$ y 1 de $1- Esp$). Asi que enves de comparar las curvas graficamente, se puede comparar sus areas bajo la curva, que es mucho mas facil. Veamos los valores del area bajo la curva para las 4 moleculas estudiadas anteriormente:
...
AUCkab<-kable(auc_res[order(auc_res$AUC),],caption="AUC para las 4 moleculas",row.names =F)
kable_styling(AUCkab,"striped",full_width = F,position = "center")

```

Al evaluar el AUC para las cuatro moleculas, vemos que la molecula 3 tiene la mayor area (en concordancia con nuestra previa inspeccion grafica). Tambien podemos observar que las diferencias en terminos de area entre las moleculas 2 y 4 es bastante pequeña (`r abs(auc_m2-auc_m4)`), siendo la molecula 2 aquella con mayor area entre las 2. Sin embargo para esta diferencia tan pequeña podemos considerar que tienen un AUC similar, validando la desicion del uso de la molecula 2 o 4 en el razonamiento de riesgo anteriormente mencionado.
