...
pf<-ggplot(data = data.frame(weight = weight_lim), aes(weight)) +
  ...
  annotate("text", x = 94, y = .015, 
           label = "P(X>q)==0.25",parse=T , size=3 , fontface="bold")+
  theme_bw()
pf
```
...
```{r qgraph2, echo=FALSE, results = 'asis',fig.width=8, fig.height=4, fig.cap="Distribucion de peso para hombres y mujeres"}
xlabels <- c(
  ~ "-5.000",
  ~ "-2.042",
  ~ "0",
