...
```
Las reglas para generar los anteriores sistemas de hipotesis son simples:

* Las hipotesis tratan de parametros como el $E(X)$ o $V(X)$. Un error comun es usar estimadores como $\overline{X}$ en el sistema de hipotesis.

* En la hipotesis nula ($H_0$) siempre debe ir una igualadad o desigualdad 

$$=~\leq~\geq$$

* En la hipotesis alternativa ($H_a$) siempre debe ir una desigualdad estricta

$$\neq~<~>$$

* por ultimo debemos escojer un valor de referencia con el cual comparar el parametro. En este caso el valor de referencia usado es 0

Como escojer cual de los tres sistemas de hipotesis es el adecuado? Eso depende del interes en la investigacion. Por ejemplo, si nos intereza demostrar que el medicamento aumenta la _glu_, entonces el sistema de hipotesis apropiado es el numero 2: $H_{0}: E(Dif)\leq 0$ vs $H_{a}: E(Dif) > 0$. 

Si nos intereza demostrar que el medicamento disminuye la _glu_ entonces el sistema apropiado es el numero 3: $H_{0}: E(Dif)\geq 0$ vs $H_{a}: E(Dif) < 0$. 

Si nos interesa mostrar que el medicamento cambia los niveles de _glu_, sin importar si el efecto es de disminucion o aumento, entonces el sistema de hipotesis apropiado es el numero 1: $H_{0}: E(Dif)=0$ vs $H_{a}: E(Dif)\neq0$. 

Por lo general, y a no ser que se especifique lo contrario, la condicion que se quiera demostrar debe ir en la hipotesis alternativa.

##Zonas de aceptacion y rechazo en funcion del sistema de hipotesis

En el ejemplo del estudio metformina + glibenclamida discutimos los limites de aceptacion y rechazo de la hipotesis nula para el sistema de hipotesis $H_{0}: E(Dif)=0$ vs $H_{a}: E(Dif)\neq0$. Sin embargo, estos limites cambian para los sistemas numero 2 y 3.

Para los efectos de esta discucion, nos concentraremos en pruebas de hipotesis que tratan del valor esperado y la varianza, es decir, los parametros a probar son $E(X)$ y $V(X)$ para una variable aleatoria $X$ cualquiera.

De esa forma, los tres sitemas de hipotesis disponibles son los siguientes:
```{r echo=FALSE, results = 'asis', message=FALSE}
textable<-"
\\begin{table}[]
\\begin{tabular}{|l|l|l|}
...
Donde $\delta$ hace referencia a un parametro cualquiera ($E(X)$ o $V(X)$) y $c$ es un valor de referencia, como por ejemplo $0$ en el ejemplo anterior.

Para el caso de $E(X)$ el estadistico de prueba que debemos usar es 

$$T_{n-1}=\frac{\sqrt{n}\Big(\overline{X}-E(X)\Big)}{S}$$
Como habiamos visto anteriormente, mientras que para $V(X)$ usaremos el estadistico chi cuadrado:

$$\chi_{n-1}^{2}= \frac{(n-1)S^2}{V(X)}$$

Notese que estos estadisticos son los mismos que usamos en la construccion de intervalos de confianza.
...
Este caso ya lo conocemos, es el mismo tratado en el ejemplo. Este sistema de hipotesis induce una zona de rechazo compuesta por dos "colas". Fijado el nivel de significancia $\alpha$ los cuantiles $t_{n-1,\alpha/2}$ y $t_{n-1,1-\alpha/2}$ o los cuantiles $\chi^{2}_{n-1,\alpha/2}$ y $\chi^{2}_{n-1,1-\alpha/2}$ para $E(X)$ y $V(X)$ respectivamente, delimitan las zonas de rechazo como se puede ver a continuacion.
...
Este sistema de hipotesis induce una zona de rechazo compuesta por una "cola". Fijado el nivel de significancia $\alpha$ el cuantil $t_{n-1,1-\alpha}$ o $\chi^{2}_{n-1,1-\alpha}$ para $E(X)$ y $V(X)$ respectivamente, delimitan las zonas de rechazo como se puede ver a continuacion.
...
Por que la zona de rechazo se encuentra a la derecha, en valores grandes de los estadisticos de prueba? La respuesta yace en las formulas de los estadisticos de prueba $T_{n-1}$ y $\chi_{n-1}^{2}$ y por supuesto en la hipotesis alternativa $H_{a}: \delta > c$. En ambas formulas de los estadisticos de prueba, valores de $E(X)$ 0 $V(X)$ mayores a $c$ se asocian con valores de los estadisticos de pruba grandes (es decir cituados a la derecha) en la distribucion de estos.

## Zona de rechazo para el sistema $H_{0}: \delta \geq c$ vs $H_{a}: \delta < c$

Este sistema de hipotesis induce una zona de rechazo compuesta por una "cola". Fijado el nivel de significancia $\alpha$ el cuantil $t_{n-1,\alpha}$ o $\chi^{2}_{n-1,\alpha}$ para $E(X)$ y $V(X)$ respectivamente, delimitan las zonas de rechazo como se puede ver a continuacion.
...
Por que la zona de rechazo se encuentra a la derecha, en valores pequeños de los estadisticos de prueba? La respuesta yace en las formulas de los estadisticos de prueba $T_{n-1}$ y $\chi_{n-1}^{2}$ y por supuesto en la hipotesis alternativa $H_{a}: \delta < c$. En ambas formulas de los estadisticos de prueba, valores de $E(X)$ o $V(X)$ menores a $c$ se asocian con valores de los estadisticos de pruba pequeños (es decir cituados a la izquierda) en la distribucion de estos.