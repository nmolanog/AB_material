...
\\begin{tabular}{|l|l|l|}
 Caso 1                 & Caso 2               & Caso 3               \\\\ \\hline
 $H_{0}: \\delta=0$      & $H_{0}: \\delta\\leq 0$ & $H_{0}: \\delta\\geq 0$ \\\\
 $H_{a}: \\delta\\neq 0$ & $H_{a}: \\delta > 0$  & $H_{a}: \\delta < 0$  \\hline
\\end{tabular}
\\end{table}
"
tex2markdown(textable)
...
Donde $\delta$ hace referencia a un parametro cualquiera ($E(X)$ o $V(X)$)
...
## Zona de rechazo para el sistema $H_{0}: \delta=0$ vs $H_{a}: \delta \neq 0$
...
chigl<-60
p2<-ggplot(data = data.frame(x = c(0,110)), aes(x)) +
  stat_function(fun = dchisq, args = list(df = chigl),color="blue")+ylab("f(x)")+xlab("x")+
  ...
  scale_x_continuous("t", round(c(-5,qchisq(1-.975,chigl),0,qchisq(.975,chigl),5),3), limits=c(-5,5))+
  annotate("segment", x = c(-2.2,2.2,0), xend = c(-3.8,3.8,2), 
           y = rep(0,3), yend = c(.16,.16,.35), colour = "red", size=1, alpha=0.6, arrow=arrow())+
  stat_function(fun = dchisq, args = list(df = chigl),
                xlim = c(qchisq(.975,chigl),5),
                geom = "area",fill="red",alpha=0.5)+
  stat_function(fun = dchisq, args = list(df = chigl),
                xlim = c(-5,qchisq(.025,chigl)),
                geom = "area",fill="red",alpha=0.5)+
  annotate("text", x = c(-3.8,3.8,2.7), y = c(0.18,0.18,.37), 
           label = c("Zona~de~rechazo~H[0]","Zona~de~rechazo~H[0]","Zona~de~aceptacion~H[0]"),parse=T , size=4 , fontface="bold")+
  theme_bw()
