...
set.seed(150)
```
En este documento exploraremos los factores que determinan la amplitud de un intervalo de confianza. Para efectos practicos, haremos el analisis con el intervalo de confianza para el valor esperado. Recordemos cual es su formula.
...
El tamaño de la muestra $n$, El nivel de confianza (es decir $1-\alpha$) y la desviacion estandar de los datos $S$. A continuacion exploraremos el efecto de cada una de estas cantidades sobre la amplitud del intervalo.

##Tamaño de la muestra

Considere la siguiente funcion de densidad, del peso para una poblacion en particular.
...
De esta poblacion hipotetica haremos diferentes muestreos con diferentes tamaños de muestra y calcularemos el intervalo de confianza para cada una. A continuacion se presentan los histogramas para cada una de las muestras:
...
La curva azul es la verdadera funcion de densidad de la poblacion. Ahora construimos los intervalos de confianza para cada muestra:
...
En azul se destaca el verdadero valor esperado de la poblacion. Es claro que a mayor tamaño de muestra, la amplitud del intervalo de confianza disminuye.

##Nivel de confianza

Para observar cual es el efecto del nivel de confianza en la amplitud del intervalo de confianza primero observemos los siguiente:

* El nivel de confianza se define como $1-\alpha$ y los cuantiles que usamos para contruir el intervalo para el valor esperado estan dados por $t_{n-1,\frac{\alpha}{2}}$ y $t_{n-1,1-\frac{\alpha}{2}}$, donde $n$ es el tamaño de la muestra.
...
A continuacion presentamos una tabla con diferentes nivels de confianza, su correspondiente $\alpha$ y cuantiles.

```{r echo=FALSE, results = 'asis'}
confianza<-c(.9,.92,.94,.95,.99)
t1<-data.frame(confianza=confianza,alpha=1-confianza,q_inf=(1-confianza)/2,q_sup=1-(1-confianza)/2)
kable_styling(kable(t1),"striped", position = "left")
```

Aqui q_inf corresponde a $\frac{\alpha}{2}$ y q_sup corresponde a $1-\frac{\alpha}{2}$.
...
Es claro que a medida que la confianza aumenta la distancia entre los correspondientes cuantiles tambien aumenta. Ahora veamos un ejemplo concreto de una muestra de pesos de una poblacion particular en donde calculamos los intervalos con diferentes valores de confianza.
